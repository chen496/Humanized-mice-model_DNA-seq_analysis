...
library(rJava)
library(stringr)
path <- 'E:/Data/excelwordcount/2/input23/'
dir.name <- dir(path)
doc.name <- list()
fre <- numeric()
...
for ( i in 1:length(dir)) {
  ...
  outfile.name[i]<-paste0('E:/Data/excelwordcount/2/output23/',str_replace(dir[i], "\\.xlsx", "\\.txt"))
  
  alin<- read.xlsx(file.name[i],sheet=1);
  ...
  n<- length(alin$Gene.refGene)
  n
  gene<- alin$Gene.refGene
  #write.csv(gene,outfile.name[i])
  write.table(gene, file = outfile.name[i], row.names = F, quote = F, col.names = F)
  
  #清洗数据，去掉空格和标点符号
  #filePath = "E:/Data/excelwordcount/foo.txt"
  outfile1.name[i]<-paste0('E:/Data/excelwordcount/2/output24/',str_replace(dir[i], "\\.xlsx", "\\.csv"))
